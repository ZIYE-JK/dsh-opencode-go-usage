 ...
 */
export async function fetchViaApiKey(cfg) {
    const apiKey = cfg.apiKey;
    if (apiKey === undefined || apiKey.length === 0) {
        throw new UsageError('未配置 OpenCode 服务账号 API Key', 'noconfig');
    }
    const url = `${cfg.baseUrl.replace(/\/+$/, '')}${GO_USAGE_PATH}`;
    const res = await getJson(url, { Authorization: `Bearer ${apiKey}`, Accept: 'application/json', 'User-Agent': USER_AGENT }, cfg.timeoutMs);
 ...
 */
export function fromGoUsage(body, nowMs) {
    const root = asRecord(body);
    if (root === undefined)
        throw new UsageError('Go usage API returned no object', 'badjson');
...
export async function fetchViaCookie(cfg) {
    ...
    if (cfg.workspaceID)
        headers['x-org-id'] = cfg.workspaceID;
    const res = await getJson(url, headers, cfg.timeoutMs);
    if (res.status === 401 || res.status === 403) {
        throw new UsageError('登录已过期：请在 opencode.ai 控制台重新登录，然后更新 Cookie（更推荐改用服务账号 API Key）', 'unauthorized');
    }
    if (res.status !== 200) {
 ...
 */
export function fromGoStatus(root, nowMs) {
    if (root === undefined)
        throw new UsageError('Console usage API returned no object', 'badjson');

...
function WindowSegment(props) {
    ...
    return (_jsxs("span", { className: css.seg, children: [sep !== undefined && sep !== '' && _jsx("span", { className: css.segSep, children: sep }), _jsxs("span", { className: cls ?? undefined, children: [WINDOW_LABELS[window.kind], " ", window.percent, "% (", formatDuration(window.resetInSec), ")"] })] }));
}
...
export function OcgoDockEntry(props) {
    ...
    const [layout, setLayout] = useState(() => window.localStorage.getItem('dsh.ocgoChip.layout') === 'col' ? 'col' : 'row');
    ...
    if (anchorRef.current === null) {
        try {
            ...
            if (raw !== null) {
                const p = JSON.parse(raw);
                if (typeof p.x === 'number' && typeof p.gap === 'number')
                    anchorRef.current = { x: p.x, gap: p.gap };
            }
        }
        catch { /* ignore corrupt storage */ }
    }
    ...
    const legacyLeftRef = useRef(null);
    if (anchorRef.current === null && legacyLeftRef.current === null) {
        try {
            const raw = window.localStorage.getItem('dsh.ocgoChip.offset');
            if (raw !== null) {
                const p = JSON.parse(raw);
                if (typeof p.x === 'number')
                    legacyLeftRef.current = p.x;
            }
        }
        catch { /* ignore corrupt storage */ }
    }
    ...
    useLayoutEffect(() => {
        ...
        cardRef.current = card;
        if (anchorRef.current === null) {
            // First run on this model (or migrating from the pixel offset): take the
            // spot the chip currently occupies as the anchor.
            const wrapRect = wrap.getBoundingClientRect();
            const cardRect = card.getBoundingClientRect();
            anchorRef.current = {
                x: legacyLeftRef.current ?? Math.round(wrapRect.left - cardRect.left),
        ...
        if (posRef.current !== null)
            setPos(null);
        // Paint before the browser draws, so the chip never flashes at the
        // stylesheet default position after a remount.
    ...
    const iconButtons = (_jsxs("span", { className: css.iconBtns, children: [_jsx("span", { className: css.lockBtn, role: "button", title: col ? t('ocgo.layoutToRow') : t('ocgo.layoutToCol'), "data-testid": "ocgo-layout-toggle", onClick: (e) => { e.stopPropagation(); toggleLayout(); }, children: col ? (_jsx("svg", { viewBox: "0 0 24 24", width: "12", height: "12", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("rect", { x: "3", y: "8.5", width: "18", height: "7", rx: "3.5" }) })) : (_jsx("svg", { viewBox: "0 0 24 24", width: "12", height: "12", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M4 6h16M4 12h16M4 18h16" }) })) }), _jsx("span", { className: css.lockBtn, role: "button", title: locked ? t('ocgo.unlock') : t('ocgo.lock'), onClick: (e) => { e.stopPropagation(); toggleLock(); }, children: locked ? (_jsx("svg", { viewBox: "0 0 24 24", width: "12", height: "12", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M18 6 6 18M6 6l12 12" }) })) : (_jsx("svg", { viewBox: "0 0 24 24", width: "12", height: "12", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M12 17v5M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V6h1a2 2 0 0 0 0-4H8a2 2 0 0 0 0 4h1z" }) })) })] }));
    ...
    return (_jsxs("span", { className: css.wrap, ref: wrapRef, "data-testid": "ocgo-chip", style: pos === null ? undefined : { left: `${pos.x}px`, top: `${pos.y}px`, bottom: 'auto' }, onMouseDown: onWrapMouseDown, children: [_jsx("button", { type: "button", className: [css.chip, col ? css.chipCol : '', open ? css.chipOpen : ''].filter((c) => c !== '').join(' '), onClick: () => {
                    ...
                    if (open)
                        ...
                        setOpen(true);
                }, title: open ? t('ocgo.collapse') : t('ocgo.expand'), children: col ? (_jsxs(_Fragment, { children: [_jsxs("span", { className: css.colHead, children: [_jsxs("span", { children: [t('ocgo.labelShort'), ":"] }), snapshot.updatedAt !== undefined && (_jsx("span", { className: css.segSep, children: t('ocgo.fetchedAt', { time: formatClock(snapshot.updatedAt) }) }))] }), windows.map((w) => (_jsx(WindowSegment, { window: w }, w.kind))), _jsxs("span", { className: css.colFoot, children: [daily !== null && (_jsx("span", { className: css.seg, children: _jsxs("span", { className: dailyCls ?? undefined, children: ["\u23F3 ", daily.toFixed(1), "%/\u5929"] }) })), iconButtons] })] })) : (_jsxs(_Fragment, { children: [_jsxs("span", { children: [t('ocgo.label'), ":"] }), windows.map((w) => (_jsx(WindowSegment, { window: w, sep: sep }, w.kind))), daily !== null && (_jsxs("span", { className: css.seg, children: [_jsx("span", { className: css.segSep, children: sep }), _jsxs("span", { className: dailyCls ?? undefined, children: ["\u23F3 ", daily.toFixed(1), "%/\u5929"] })] })), snapshot.updatedAt !== undefined && (_jsxs("span", { className: css.segSep, children: [sep, t('ocgo.fetchedAt', { time: formatClock(snapshot.updatedAt) })] })), iconButtons] })) }), open && (_jsx("span", { className: css.details, children: mode === 'set' ? (_jsxs("span", { className: css.setPanel, children: [_jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.workspaceID') }), _jsx("input", { className: css.fieldInput, value: wsDraft, placeholder: "wrk_\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setWsDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.workspaceID))
                                        e.target.select(); } })] }), _jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('ocgo.cookie') }), _jsx("input", { className: css.fieldInput, value: cookieDraft, placeholder: "auth=\u2026", spellCheck: false, autoComplete: "off", onChange: (e) => { setCookieDraft(e.target.value); }, onFocus: (e) => { if (e.target.value === maskedText(config?.cookie))

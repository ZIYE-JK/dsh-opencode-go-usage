...
function dailyRemaining(window: UsageWindow | undefined): number | null {
  if (window === undefined || window.resetInSec <= 0) return null
...
function dailySeverityClass(daily: number): string | undefined {
  if (daily < 3) return css.segErr
  if (daily < 5) return css.segWarn
  return css.segOk
}
...
export function OcgoDockEntry(props: OcgoDockEntryProps): React.ReactElement | null {
  ...
  const captureAnchor = (card: HTMLElement): void => {
  ...
  const applyPosition = (wrap: HTMLElement): void => {
  ...
  if (error !== null) {
    return (
      <span
        ...
        style={pos === null ? undefined : { left: `${pos.x}px`, top: `${pos.y}px`, bottom: 'auto' }}
